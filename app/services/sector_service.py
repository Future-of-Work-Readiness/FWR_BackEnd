...
from app.models import Sector, Branch, Specialization
...
class SectorService:
    ...
    def get_specializations_by_sector(db: Session, sector_id: UUID) -> List[Specialization]:
        ...
        return db.query(Specialization).filter(
            Specialization.branch_id.in_(branch_ids)
        ).all()
    ...
    @staticmethod
    def get_specialization_by_id(db: Session, specialization_id: UUID, active_only: bool = True) -> Optional[Specialization]:
