...
import json
from app.models import User, PeerBenchmark, Specialization
...
class BenchmarkService:
    ...
    def calculate_peer_benchmarks(db: Session, specialization_id: UUID) -> bool:
        ...
        existing = db.query(PeerBenchmark).filter(
            PeerBenchmark.specialization_id == specialization_id
        ).first()
    ...
    def get_peer_benchmark(db: Session, user_id: UUID) -> Optional[Dict[str, Any]]:
        ...
        benchmark = db.query(PeerBenchmark).filter(
            PeerBenchmark.specialization_id == user.preferred_specialization_id
        ).first()
        ...
        specialization = db.query(Specialization).filter(
            Specialization.specialization_id == user.preferred_specialization_id
        ).first()
