...
from app.services import SectorService
...
def get_complete_hierarchy(db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching complete hierarchy: {str(e)}"
        )
...
def get_branches_by_sector(sector_id: UUID, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching branches: {str(e)}"
        )
...
def get_sector_full_hierarchy(sector_id: UUID, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching sector hierarchy: {str(e)}"
        )
...
def get_specializations_by_branch(branch_id: UUID, db: Session = Depends(get_db)):
    ...
    try:
        ...
        specializations = SectorService.get_specializations_by_branch(
            db, branch_id, active_only=True
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching specializations: {str(e)}"
        )
...
def get_specialization_by_id(specialization_id: UUID, db: Session = Depends(get_db)):
    ...
    try:
        specialization = SectorService.get_specialization_by_id(
            db, specialization_id, active_only=True
        )
        ...
        return {
            ...
            "created_at": (
                specialization.created_at.isoformat()
                if specialization.created_at
                else None
            ),
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching specialization: {str(e)}"
        )
