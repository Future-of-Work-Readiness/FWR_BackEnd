"""
Goal and Journal Entry API endpoints - v1
"""
...
@router.post("/")
def create_goal(
    goal: schemas.GoalCreate, user_id: UUID = Query(...), db: Session = Depends(get_db)
):
...
def update_goal_progress(
    ...
    goal = GoalService.update_goal_progress(
        db=db, goal_id=goal_id, user_id=user_id, current_value=current_value
    )
    if not goal:
        raise HTTPException(status_code=404, detail="Goal not found")
    return {
...
def create_journal_entry(
    ...
    db_entry = GoalService.create_journal_entry(
        db=db, user_id=user_id, content=entry.content, prompt=entry.prompt
    )
    return {
...
def get_journal_entries(
    user_id: UUID = Query(...), limit: int = Query(20), db: Session = Depends(get_db)
):
...
def update_journal_entry(
    ...
    updated_entry = GoalService.update_journal_entry(
        db, entry_id, user_id, entry_update.content
    )
...
def delete_journal_entry(
    entry_id: UUID, user_id: UUID = Query(...), db: Session = Depends(get_db)
):
