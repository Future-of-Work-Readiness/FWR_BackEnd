...
from app.services import QuizService, GoalService, BenchmarkService, UserService
        ...
        quiz_list.append(
            {
                ...
                "specialization_name": (
                    quiz.specialization.name if quiz.specialization else None
                ),
...
def get_quizzes_by_specialization(
    specialization_id: UUID, db: Session = Depends(get_db)
):
...
def submit_quiz(
    attempt_id: UUID, data: schemas.QuizSubmission, db: Session = Depends(get_db)
):
    ...
    updated_goals = []
    if attempt:
        updated_goals = GoalService.auto_update_goals_on_quiz_completion(
            db, attempt.user_id
        )
        if updated_goals:
