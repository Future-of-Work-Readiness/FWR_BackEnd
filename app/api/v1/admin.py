...
from app.services import SectorService, UserService, QuizService
from app.models import Sector, Branch, Specialization, User, Quiz
...
def get_statistics(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "active_specializations": db.query(Specialization)
        ...
        result.append(
            {
                ...
                "created_at": (
                    sector.created_at.isoformat() if sector.created_at else None
                ),
            }
        )
    return result
...
def get_all_branches(sector_id: Optional[UUID] = None, db: Session = Depends(get_db)):
    ...
    branches = SectorService.get_all_branches(
        db, sector_id=sector_id, active_only=False
    )
...
def create_branch(branch: BranchCreate, db: Session = Depends(get_db)):
    ...
    new_branch = SectorService.create_branch(
        db, branch.name, branch.sector_id, branch.description
    )
...
def get_all_specializations(
    branch_id: Optional[UUID] = None, db: Session = Depends(get_db)
):
    """Get all specializations, optionally filtered by branch."""
    specializations = SectorService.get_all_specializations(
        db, branch_id=branch_id, active_only=False
    )
    ...
    for spec in specializations:
        ...
        sector = (
            SectorService.get_sector_by_id(db, branch.sector_id, active_only=False)
            if branch
            else None
        )
...
def create_specialization(spec: SpecializationCreate, db: Session = Depends(get_db)):
    ...
    new_spec = SectorService.create_specialization(
        db, spec.name, spec.branch_id, spec.description
    )
...
def update_specialization(
    spec_id: UUID, spec: SpecializationUpdate, db: Session = Depends(get_db)
):
...
def get_all_users(db: Session = Depends(get_db)):
    ...
    for user in users:
        ...
        if user.preferred_specialization_id:
            spec = SectorService.get_specialization_by_id(
                db, user.preferred_specialization_id, active_only=False
            )
        ...
        result.append(
            {
                ...
                "preferred_specialization_id": str(user.preferred_specialization_id) if user.preferred_specialization_id else None,
