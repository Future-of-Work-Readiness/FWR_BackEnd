...
class ReadinessSnapshot(BaseModel):
    """Snapshot of user's readiness scores (inline to avoid circular import)"""
    overall: float
    technical: float
    soft: float
...
class FeedbackDetail(BaseModel):
    """Detailed feedback for quiz performance (inline to avoid circular import)"""
    overall: str
    strengths: str
    weaknesses: str
...
class ScoreImpact(BaseModel):
    """Impact of quiz on user scores (inline to avoid circular import)"""
    category: str
    old_score: float
    new_score: float
    increase: float
